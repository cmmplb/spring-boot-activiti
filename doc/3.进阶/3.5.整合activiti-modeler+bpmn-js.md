...
2. 流程设计接口迁移：
    ...
    - 考虑到后面整合 bpmn-js，把 modeler 的单独存放，以后项目整合要是来找示例也方便，把之前写在 ModelController
      的保存流程设计和获取模型流程设计挪到 ModelerController 中，记得删除一下原来 ModelController 和 Service 中的接口：
...
1. 补充字段 description 和 author

想了一下，在 model/index 和 model/form-model 中都加一个 description/author 字段显示和编辑，一开始没加的原因是描述是存储在
metaInfo 中，而 author 存储在模型文件中，有点麻烦，这里列表处理一下描述回显，编辑的时候处理作者的回显

- 在接口返回的 ModelVO 添加 description 字段，添加 getDescription 方法来重写获取逻辑：
...
- 修改模型接口补充更新流程设计文件信息
  ...
  `io.github.cmmplb.activiti.service.impl.ModelServiceImpl.update()`
...
- 模型详情接口补充作者信息：
  ...
  `io.github.cmmplb.activiti.service.impl.ModelServiceImpl.getInfoById()`
...
2. 保存模型的时候，作者字段取错了...
   ...
   `io.github.cmmplb.activiti.service.impl.ModelServiceImpl.save()`
